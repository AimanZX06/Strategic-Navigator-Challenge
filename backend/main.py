...
try:
    ...
    # print(f"Model loaded. 2026 Target: {target_2026:.2f} gCO2/MJ") # COMMENTED OUT
    print(f"Model loaded. 2026 Target: {target_2026:.2f} kg/nm")
...
def predict_co2(ship: ShipInput):
    ...
    # Note: Even though we don't use fuel_type for math anymore, we keep passing it
    # if the model was trained with it. If 'train_model.py' removed fuel_type from X,
    # it won't be used here, but it doesn't hurt to leave it in the input dataframe.
    input_data = pd.DataFrame([{
        'ship_type': ship.ship_type,
        'distance': ship.distance,
        'fuel_consumption': ship.fuel_consumption,
        # 'fuel_type': ship.fuel_type # Optional: Comment out if your new model doesn't use it
    }])
    ...
    # Formula: kg CO2 / Nautical Mile
    if ship.distance > 0:
        ghg_intensity = predicted_co2_kg / ship.distance
    else:
        ghg_intensity = 0
